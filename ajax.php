...
$PAGE->set_url(new moodle_url('/course/format/mypdchat/ajax.php', array('id' => $courseid)));

require_once($CFG->dirroot . '/course/format/mypdchat/locallib.php');

require_once($CFG->dirroot . '/course/format/mypdchat/classes/local/action_handler.php');

switch ($action) {

    case 'postcomment' :

        $result = \format_mypdchat\local\action_handler::post_comment($course);

        if ($result['error'] == 0) {

            $renderer = $PAGE->get_renderer('format_mypdchat');
            $comment = $DB->get_record('format_mypdchat_comments', array('id' => $result['commentid']));
            $post = $DB->get_record('format_mypdchat_posts', array('id' => $comment->postid));
    ...
    case 'deletecomment':
        ...
        $result = \format_mypdchat\local\action_handler::delete_comment();
        echo json_encode($result);
        die;
    ...
    case 'likepost' :
        ...
        $result = \format_mypdchat\local\action_handler::like_post($course);
        echo json_encode($result);
        die;

    case 'lockpost':

        $result = \format_mypdchat\local\action_handler::lock_post($course);
        echo json_encode($result);
        die;
    ...
    case 'showallcomments':
        ...
        if ($post->courseid <> $course->id) {
            print_error('invalidcourseid', 'format_mypdchat');
        }

        $limitreplies = ($action == 'showalldiscussions') ? 0 : $course->tlnumreplies;

        $comments = \format_mypdchat\local\comments::instance();
    ...
    case 'showallreplies':
        ...
        if (!$comment = $DB->get_record('format_mypdchat_comments', array('id' => $replycommentid))) {
            print_error('invalidcommentid', 'format_mypdchat');
        }
