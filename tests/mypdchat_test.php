...
class format_mypdchat_testcase extends advanced_testcase {
    ...
    private function create_post_with_attachment($course1id, $userid, $attachmentid) {
        ...
        \format_mypdchat\local\attaches::save_attaches($post->id, $post->cmsequence);
    ...
    public function test_unenroluser() {
        ...
        require_once($CFG->dirroot . '/course/format/mypdchat/lib.php');
        ...
        // Check post.
        $count = $DB->count_records('format_mypdchat_posts', array('fromuserid' => $user1->id));
        ...
        $DB->set_field('course_format_options', 'value', 1, array('format' => 'mypdchat', 'name' => 'deleteafterunenrol'));

        format_mypdchat::reset_course_cache();
        $course1 = course_get_format($course1)->get_course();
        $this->assertEquals(1, $course1->deleteafterunenrol);

        // Set deletemodspermanently. 
        $DB->set_field('course_format_options', 'value', 1, array('format' => 'mypdchat', 'name' => 'deletemodspermanently'));
        ...
        // Check post.
        $count = $DB->count_records('format_mypdchat_posts', array('fromuserid' => $user1->id, 'courseid' => $course1->id));
        $this->assertEquals(0, $count);

        $count = $DB->count_records('label', array('id' => $label1->id));
        $this->assertEquals(0, $count);

        $count = $DB->count_records('format_mypdchat_posts', array('fromuserid' => $user1->id, 'courseid' => $course2->id));
