...
class addactivity_form extends moodleform {
    ...
    protected function definition() {
        ...
        if (!empty($typeelements)) {
            ...
            // Module type filter.
            $mform->addElement('header', 'filtersheader', get_string('filtersheader', 'format_mypdchat'));
            $mform->addGroup($typeelements, 'filterbytype');

            $params = array('size' => '30', 'placeholder' => get_string('searchbyname', 'format_mypdchat'));
            $mform->addElement('text', 'searchbyname', '', $params);
            $mform->setType('searchbyname', PARAM_TEXT);

            // Recent activities area.
            $mform->addElement('header', 'recentactivitiesheader', get_string('recentactivities', 'format_mypdchat'));
            ...
            $mform->addElement('html', get_string('norecentactivities', 'format_mypdchat'));
        }
        ...
        $PAGE->requires->strings_for_js(
                array('addrecentactivity', 'attach', 'cancel'), 'format_mypdchat');

        $PAGE->requires->yui_module(
                'moodle-format_mypdchat-addactivity', 'M.format_mypdchat.addactivityinit', array($args), null, true);
    }
