...
class editnotification_form extends moodleform {
    ...
    protected function definition() {
        ...
        $choices = array();
        foreach (\format_mypdchat\local\notification::$NOTIFICATION_TYPE as $key => $type) {
            $choices[$key] = get_string($type, 'format_mypdchat');
        }

        $mform->addElement('select', 'notificationtype', get_string('notificationtype', 'format_mypdchat'), $choices);
        ...
        $this->add_action_buttons(true, get_string('savesetting', 'format_mypdchat'));
    }
