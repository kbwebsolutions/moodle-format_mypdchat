...
class restore_format_mypdchat_plugin extends restore_local_plugin {
    ...
    public function process_post($data) {
        ...
        $newid = $DB->insert_record('format_mypdchat_posts', $data);

        $this->set_mapping('mypdchatpost', $oldid, $newid);
    }
    ...
    public function process_like($data) {
        ...
        $data = (object) $data;
        $data->courseid = $this->task->get_courseid();
        $data->postid = $this->get_mappingid('mypdchatpost', $data->postid);
        $data->fromuserid = $this->get_mappingid('user', $data->fromuserid);

        $DB->insert_record('format_mypdchat_likes', $data);
    }
    ...
    public function process_comment($data) {
        ...
        $newid = $DB->insert_record('format_mypdchat_comments', $data);
        $this->set_mapping('mypdchatcomment', $oldid, $newid);
    }
    ...
    public function after_restore_course() {
        ...
        if (!$childcomments = $DB->get_records_select('format_mypdchat_comments', " courseid = ? and replycommentid > 0", array($courseid))) {
            return;
        }

        foreach ($childcomments as $comment) {

            $comment->replycommentid = $this->get_mappingid('mypdchatcomment', $comment->replycommentid);
    ...
    public function process_attachment($data) {
        ...
        $DB->insert_record('format_mypdchat_attaches', $data);
    }
