YUI.add('moodle-format_mypdchat-postform', function (Y, NAME) {

M.format_mypdchat = M.format_mypdchat || {};

M.format_mypdchat.postforminit = function (data) {
    "use strict";
    ...
    function callbackPostliked(responsedata) {
        ...
        if (responsedata.userlike == '1') {
            ...
            likelink.setHTML(M.str.format_mypdchat.likenomore);

        } else {
            // like
            likelink.replaceClass('likenomore', 'like');
            likelink.setHTML(M.str.format_mypdchat.like);
        }
    ...
    function callbackMorePostsLoaded(responsedata) {
        ...
        var counttotalpost = M.str.format_mypdchat.counttotalpost.replace('{$a->count}', data.postsloaded)
    ...
    function callbackPostLocked(responsedata) {
        ...
        if (responsedata.locked == '1') {
            ...
            icon.set('src', M.util.image_url('lockedpost', 'format_mypdchat'));

        } else {

            showlink.show();
            linknode.replaceClass('locked', 'unlocked');
            M.util.image_url('unlockedpost', 'format_mypdchat');
            icon.set('src', M.util.image_url('unlockedpost', 'format_mypdchat'));
        }
    }
    ...
    function onClickPostComment(postbutton) {
        ...
        if (!text) {
            alert(M.str.format_mypdchat.textrequired);
            return false;
        }
    ...
    function updatePostCounts(responsedata) {
        ...
        if (countcomments) {
            countcomments.setHTML(M.str.format_mypdchat.countcomments.replace('{$a}', responsedata.countcomments));
        }

        var countlikes = Y.one('#tlcountlikes_' + responsedata.postid);
        if (countlikes) {
            countlikes.setHTML(M.str.format_mypdchat.countlikes.replace('{$a}', responsedata.countlikes));
        }
    }
    ...
    function onClickDeleteComment(linknode) {
        ...
        var confirm = new M.core.confirm({
            ...
            question: M.util.get_string('confirmdeletecomment', 'format_mypdchat'),
