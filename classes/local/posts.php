...
class posts {
     ...
     * 
     * @param int $courseid
     * @return \format_mypdchat\local\posts
     */
    ...
    public static function cleanup_userdeleted($userid) {
        ...
        $DB->delete_records('format_mypdchat_comments', array('fromuserid' => $userid));

        // ... delete likes.
        $DB->delete_records('format_mypdchat_likes', array('fromuserid' => $userid));

        // ... delete all the enqueued notifications.
        $DB->delete_records('format_mypdchat_nfqueue', array('recipientid' => $userid));
        $DB->delete_records('format_mypdchat_nfqueue', array('creatorid' => $userid));

        // ... delete all the notification settings.
        $DB->delete_records('format_mypdchat_nfsettings', array('userid' => $userid));
    ...
    public static function cleanup_userunrenolled($userid, $courseid) {
        ...
        // ... delete comments of user.
        $DB->delete_records_select('format_mypdchat_comments', 'fromuserid = ? AND courseid = ?', array($userid, $courseid));

        return true;
    }

    /**
     * Cleanup all the data in the format mypdchat tables 
     * 
    ...
    public static function cleanup_coursedeleted($courseid) {
        ...
        // ... delete post.
        $DB->delete_records('format_mypdchat_posts', array('courseid' => $courseid));

        // ... delete comments.
        $DB->delete_records('format_mypdchat_comments', array('courseid' => $courseid));

        // ... delete likes.
        $DB->delete_records('format_mypdchat_likes', array('courseid' => $courseid));

        // ... delete all the enqueued notifications.
        $DB->delete_records('format_mypdchat_nfqueue', array('courseid' => $courseid));

        // ... delete all the notification settings.
        $DB->delete_records('format_mypdchat_nfsettings', array('courseid' => $courseid));
    }
    ...
    public function get_post_form($postsdata = null) {
        ...
        // Count the activities in section == FORMAT_MYPDCHAT_POSTFORMSECTION.
        $modinfo = get_fast_modinfo($this->courseid);
        $cmids = (isset($modinfo->sections[FORMAT_MYPDCHAT_POSTFORMSECTION])) ? $modinfo->sections[FORMAT_MYPDCHAT_POSTFORMSECTION] : array();

        $options = $this->get_filter_options($this->courseid);
        $formparams = array('courseid' => $this->courseid, 'cmids' => $cmids, 'options' => $options);

        $actionurl = new \moodle_url('/course/format/mypdchat/action.php');
    ...
    protected function get_filter_options($courseid) {
        ...
        $cache = \cache::make('format_mypdchat', 'timelinefilter');
    ...
    protected function get_all_posts($course, $options = null, $limitfrom = 0, $limitcount = 0, $orderby = array()) {
        ...
        // ... no private posts?
        if (!has_capability('format/mypdchat:viewprivate', $context)) {
            $cond[] = " sp.private = '0'";
        }
        ...
        if (!empty($options->filtermodules)) {
            $join = "JOIN {format_mypdchat_attaches} at ON at.postid = sp.id ";
        ...
        // ... get all posts.
        $sqlfrom = "FROM {format_mypdchat_posts} sp {$join} {$where} ";
    ...
    private function check_and_move_module($courseid, $cmsequence) {
        ...
        $section = $modinfo->get_section_info(FORMAT_MYPDCHAT_TIMELINESECTION);
    ...
    public function save_post($data, $course) {
        ...
        if (!empty($data->id)) {
            ...
            if ($exists = $DB->get_record('format_mypdchat_posts', array('id' => $data->id))) {
                $post = $exists;

                // Check, whether user is allowed to update the post.
                $caneditpost = (($post->fromuserid == $USER->id) and ( has_capability('format/mypdchat:updateownpost', $context)));
                $caneditpost = ($caneditpost or has_capability('format/mypdchat:updateanypost', $context));

                if (!$caneditpost) {
                    print_error('missingcapupdatepost', 'format_mypdchat');
                }
                $update = true;
            } else {
                print_error('noposttoupdate', 'format_mypdchat');
            }
        }
        ...
        if (!$PAGE->user_allowed_editing()) {
            ...
            if (!empty($data->files)) {
                ...
                if (!$canpostfile) {
                    print_error('missingcappostfile', 'format_mypdchat');
                }
                ...
                if (!empty($data->externalurl)) {
                    ...
                    if (!$canposturl) {
                        print_error('missingcapposturl', 'format_mypdchat');
                    }
        ...
        if ((empty($data->posttext)) and ( empty($cmsequence))) {
            print_error('attachmentorpostrequired', 'format_mypdchat');
        }
        ...
        if (has_capability('format/mypdchat:posthtml', $context)) {
        ...
        if ($update) {
            ...
            $post->id = $DB->insert_record('format_mypdchat_posts', $post);
        }
    ...
    public function delete_post($pid) {
        ...
        if (!$post = $DB->get_record('format_mypdchat_posts', array('id' => $pid))) {
            print_error('postidinvalid', 'format_mypdchat');
        }
        ...
        if (!$candeletepost) {
            print_error('missingcapdeletepost', 'format_mypdchat');
        }
    ...
    protected static function delete_users_posts_activities($userid, $courseid = 0) {
        ...
        $where = " WHERE o.format = 'mypdchat' AND o.name = 'deletemodspermanently' AND o.value = '1' ";
        ...
        $sql = "SELECT a.coursemoduleid, count(*) as countmod
                 FROM {format_mypdchat_attaches} a
                 JOIN {format_mypdchat_posts} p ON (p.id = a.postid AND p.fromuserid = :userid)
                 JOIN {format_mypdchat_attaches} a2 ON (a2.coursemoduleid = a.coursemoduleid AND a2.postid = p.id)
    ...
    protected static function delete_users_posts($userid, $courseid = 0) {
        ...
        // ... delete comments.
        $sql = "DELETE c FROM {format_mypdchat_comments} c
                JOIN mdl_format_mypdchat_posts p ON p.id = c.postid {$where}";

        $DB->execute($sql, $params);

        // ... delete likes.
        $sql = "DELETE l FROM {format_mypdchat_likes} l
                JOIN mdl_format_mypdchat_posts p ON p.id = l.postid {$where}";
        ...
        // ... delete attaches.
        $sql = "DELETE a FROM {format_mypdchat_attaches} a
                JOIN mdl_format_mypdchat_posts p ON p.id = a.postid {$where}";

        $DB->execute($sql, $params);

        // ... delete all the enqueued notifications about this post.
        $sql = "DELETE q FROM {format_mypdchat_nfqueue} q
                JOIN mdl_format_mypdchat_posts p ON p.id = q.postid {$where}";
        ...
        $DB->delete_records('format_mypdchat_posts', $params);
    }
    ...
    protected static function delete_posts_activities($pid) {
        ...
        // Get the course and check, whether the format is "mypdchat".
        $sql = "SELECT c.* FROM {course} c
                JOIN {format_mypdchat_posts} p ON p.courseid = c.id
                WHERE p.id = :pid AND c.format = 'mypdchat'";
        ...
        $sql = "SELECT a.coursemoduleid, count(*) as countmod
                 FROM {format_mypdchat_attaches} a
                 JOIN {format_mypdchat_posts} p ON (p.id = a.postid AND p.courseid = :courseid)
                 JOIN {format_mypdchat_attaches} a2 ON (a2.coursemoduleid = a.coursemoduleid AND a2.postid = :pid)
                 GROUP BY coursemoduleid
                 HAVING countmod = '1'";
    ...
    protected static function execute_delete($pid) {
        ...
        // ... delete comments.
        $DB->delete_records('format_mypdchat_comments', array('postid' => $pid));

        // ... delete likes.
        $DB->delete_records('format_mypdchat_likes', array('postid' => $pid));

        // ... delete activities attached to this post only?
        self::delete_posts_activities($pid);

        // ... delete attaches.
        $DB->delete_records('format_mypdchat_attaches', array('postid' => $pid));

        // ... delete all the enqueued notifications.
        $DB->delete_records('format_mypdchat_nfqueue', array('postid' => $pid));

        // ... delete post.
        $DB->delete_records('format_mypdchat_posts', array('id' => $pid));
    }

    /**
     * Refresh the value of the countcomments column in format_mypdchat table
     * 
     * @param int $postid
     */
    ...
    /**
     * Refresh the value of the countlikes column in format_mypdchat table
     * 
     * @param int $postid
     */
    ...
    public function save_posts_locked_from_submit() {
        ...
        if (!has_capability('format/mypdchat:lockcomment', $coursecontext)) {
            print_error('missingcaplockcomment', 'format_mypdchat');
        }
