...
class likes {
    ...
    public function add_likes_to_posts(&$postsdata) {
        ...
        $sql = "SELECT postid FROM {format_mypdchat_likes}
    ...
    public function save_likes_from_submit() {
        ...
        if (empty($this->course->enablelikes)) {
            print_error('likesaredisabled' , 'format_mypdchat');
        }
        ...
        if (!has_capability('format/mypdchat:like', $coursecontext)) {
            print_error('missingcaplikepost', 'format_mypdchat');
        }

        $refresh = false;
        if (empty($userlike)) {

            if ($like = $DB->get_records('format_mypdchat_likes', array('postid' => $postid, 'fromuserid' => $USER->id))) {

                $DB->delete_records_select('format_mypdchat_likes', 'fromuserid = ? AND postid = ?', array($USER->id, $postid));
                $refresh = true;
            ...
            if (!$like = $DB->get_records('format_mypdchat_likes', array('postid' => $postid, 'fromuserid' => $USER->id))) {
                ...
                $DB->insert_record('format_mypdchat_likes', $newlike);
                $refresh = true;
