...
define('mypdchat_NOTIFICATION_NO', 0);
define('mypdchat_NOTIFICATION_INSTANT', 1);
define('mypdchat_NOTIFICATION_DIGEST', 2);
...
class notification {
    ...
    public static $NOTIFICATION_TYPE = array(
        mypdchat_NOTIFICATION_NO => 'notificationoff',
        mypdchat_NOTIFICATION_INSTANT => 'notificationperpost',
        mypdchat_NOTIFICATION_DIGEST => 'notificationdigest');
    ...
    public static function enqueue_post_created($post) {
        ...
        $eventdata->time = time();
        $DB->insert_record('format_mypdchat_nfqueue', $eventdata);
        return true;
    }
    ...
    protected static function get_enqueued_notifications($starttime, $endtime) {
        ...
        $sql = "SELECT q.*, p.posttext, p.private, p.togroupid
                FROM {format_mypdchat_nfqueue} q
                JOIN {format_mypdchat_posts} p ON p.id = q.postid
    ...
    protected function get_users_to_notify($notification, $course, $context) {
        ...
        if ($notification->private == 1) {
            $capability[] = 'format/mypdchat:viewprivate';
        }

        if ($notification->module == 'like') {
            $capability[] = 'format/mypdchat:viewlikes';
        }
    ...
    protected function add_users_notificationsetting(&$users, $courseid) {
        ...
        $counts = array();
        $counts[mypdchat_NOTIFICATION_INSTANT] = 0;
        $counts[mypdchat_NOTIFICATION_DIGEST] = 0;
        ...
        return $counts[mypdchat_NOTIFICATION_INSTANT];
    }
    ...
    protected function get_notifications_plaintext($notification) {
        ...
        if ($notification->module == 'comment') {
            $comment = $DB->get_record('format_mypdchat_comments', array('id' => $notification->details));
        ...
        $this->notificationsplaintext[$notification->id][$notification->details] =
                get_string($langkeymessage, 'format_mypdchat', $a);
    ...
    protected function create_message($notification, $course) {
        ...
        $eventdata->component = 'format_mypdchat';
        ...
        $eventdata->subject = get_string('notificationfromcourse', 'format_mypdchat', $course->shortname);
        ...
        $courselink = \html_writer::link($url, get_string('nf_gotocourse', 'format_mypdchat'));
    ...
    protected function postpone_notification($notification, $touser) {
        ...
        $DB->insert_record('format_mypdchat_nfqueue', $notification);
    }
    ...
    public function instant_cron() {
        ...
        $config = get_config('format_mypdchat');
        ...
        foreach ($notifications as $notification) {
            ...
            $DB->delete_records_select('format_mypdchat_nfqueue', 'id = ?', array($notification->id));
            ...
            foreach ($userstonotificate as $user) {
                ...
                if ($user->enablenotification == mypdchat_NOTIFICATION_INSTANT) {
                    ...
                    if ($user->enablenotification == mypdchat_NOTIFICATION_DIGEST) {
                        $postponed++;
        ...
        set_config('lastinstantcron', $timenow, 'format_mypdchat');
        mtrace('format_mypdchat instant_cron succesfully finished.');
    }
    ...
    public function digest_cron() {
        ...
        foreach ($groupednotifications as $userid => $usersnotifications) {
            ...
            foreach ($usersnotifications as $courseid => $coursenotifications) {
                ...
                $url = new \moodle_url('/course/view.php', array('id' => $notification->courseid));
                $digesttexthtml .= '<br>' . \html_writer::link($url, get_string('nf_gotocourse', 'format_mypdchat'));
            }
            ...
            $eventdata->subject = get_string('timelinedigests', 'format_mypdchat');   // Very short one-line subject.
            ...
            if (message_send($eventdata)) {
                ...
                $DB->delete_records_select('format_mypdchat_nfqueue', 'recipientid = ?', array($userid));
                $countdigests++;
            } else {
                $counterrors++;
            }
        ...
        set_config('lastdigestcron', $timenow, 'format_mypdchat');
        mtrace('format_mypdchat digest_cron successfully finished (' . $countdigests . ' sent, ' . $counterrors . ' Errors).');
    }
    ...
    public function save_from_submit($data) {
        ...
        if ($exists = $DB->get_record('format_mypdchat_nfsettings', $params)) {

            $exists->notificationtype = $data->notificationtype;
            $DB->update_record('format_mypdchat_nfsettings', $exists);
        } else {

            $DB->insert_record('format_mypdchat_nfsettings', $data);
        }
    }
    ...
    public function get_notification_settings($courseid) {
        ...
        $sql = "SELECT userid, notificationtype FROM {format_mypdchat_nfsettings} WHERE courseid = ?";
